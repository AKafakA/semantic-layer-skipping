...
class SemanticSkipRunner:
    ...
    def infer_and_populate(
        ...
        *,
        ...
        logging.info(
            f"Last Tokens: {self.model.to_string(tokens[0, -1])}"
        )  # print the last token string
        ...
        for checkpoint_idx, layer_idx in enumerate(self.checkpoints):
            ...
            if early_exit_strategy:
                ...
                if should_exit:
                    ...
                    logging.info(f"  [L{layer_idx}] Added EXIT decision.")
            ...
            for future_checkpoint_idx in range(
                ...
                if is_valid_skip:
                    ...
                    logging.info(f"  [L{layer_idx}] Added SKIP to L{target_layer_idx}")
                    # found the biggest skip,
                    # so stop searching for this start of checkpoint
                    break

        return target_token_str

    def infer_with_skipping(
        self, prompt: str, vector_db: SkippingVectorDB, threshold: float = 0.7
    ) -> str:
        """
        ...
        Returns the final prediction.
        """
        logging.info(f"Inference with Skipping: '{prompt[:30]}...'")
        ...
        def checkpoint_hook(resid_pre, hook):
            ...
            if result:
                ...
                similarity, decision = result.similarity, result.decision

                if similarity < threshold:
                    return resid_pre  # continue
                    ...
                    target_ckpt_idx = current_ckpt_idx + decision.skip_count
        ...
        for layer_idx in self.checkpoints:
            ...
            fwd_hooks.append((hook_name, checkpoint_hook))
        ...
        pred_str = self.model.to_string(pred_token)

        logging.info(
            f"Final Prediction: '{pred_str}' |"
            f" Skipped Layers: {ctx.skipped_layers_count}"
        )
        return pred_str
